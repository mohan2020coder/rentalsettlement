...
func RegisterRoutes(group *gin.RouterGroup, db *gorm.DB, tenancySvc *tenancies.Service, deductRepo *deductions.Repository, auditSvc *audit.Service, notify *notifications.Service) {
	repo := NewRepository(db)
	svc := NewService(repo, tenancySvc, deductRepo, auditSvc, notify)
	h := NewHandler(svc)
